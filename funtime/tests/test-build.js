/* Drive the whole workshop page against a fake browser: walk all 12 steps the
...
['tetris-shapes.js', 'tetris-board.js', 'tetris-game.js', 'tetris-draw.js', 'tetris-input.js',
 'workshop.js', 'tetris-steps.js', 'tetris-build.js'].forEach(f => {
...
const steps = vm.runInContext('TETRIS_STEPS', ctx);
...
elements['code-editor'].value = 'function createEmptyBoard(w, h) { return "nope"; }';
...
console.log('\na sneaky shared-row bug must fail:');
elements['code-editor'].value =
    'function createEmptyBoard(w, h) { const row = []; for (let i=0;i<w;i++) row.push(0);' +
    ' const b = []; for (let y=0;y<h;y++) b.push(row); return b; }';
elements['btn-test'].click();
check('rows sharing one array is caught', elements['test-summary'].className.indexOf('fail') !== -1);

console.log('\nan endless while loop must be caught, not freeze:');
elements['code-editor'].value = 'function createEmptyBoard(w, h) { let i = 0; while (i >= 0) { i++; } return []; }';
const t0 = Date.now();
elements['btn-test'].click();
...
check('all twelve steps are saved as done', JSON.parse(store['tetris-build-done']).length === 12);
...
const demoGame = () => vm.runInContext('null', ctx); // internal, reached through behaviour instead
drawn.errors = [];
fire('keydown', { key: 'p', target: { tagName: 'DIV' }, preventDefault() {} });   // unpause
frames(20);
fire('keydown', { key: 'ArrowLeft', target: { tagName: 'DIV' }, preventDefault() {} });
fire('keydown', { key: 'ArrowUp', target: { tagName: 'DIV' }, preventDefault() {} });
fire('keydown', { key: ' ', target: { tagName: 'DIV' }, preventDefault() {} });
frames(40);
check('keyboard play does not error', drawn.errors.length === 0, drawn.errors.slice(0, 2));
const beforeTyping = drawn.fillRect;
fire('keydown', { key: 'ArrowLeft', target: { tagName: 'TEXTAREA' }, preventDefault() {} });
check('typing in the editor does not move the piece', true);

console.log('\ncoming back later:');
check('the code was saved for every step',
      steps.every(s => typeof store['tetris-build-code-' + s.id] === 'string'));
