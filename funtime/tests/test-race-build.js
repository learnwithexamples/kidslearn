/* Drive the whole RACING workshop page against a fake browser: walk all 12 steps the
...
['race-road.js', 'race-game.js', 'race-draw.js', 'race-input.js',
 'workshop.js', 'race-steps.js', 'race-build.js'].forEach(f => {
...
elements['code-editor'].value = 'function laneCenterX(lane) { return "nope"; }';
...
console.log('\nforgetting to centre the lane must fail:');
elements['code-editor'].value = 'function laneCenterX(lane) { return EDGE_WIDTH + lane * LANE_WIDTH; }';
elements['btn-test'].click();
check('a lane edge instead of a lane middle is caught', elements['test-summary'].className.indexOf('fail') !== -1);

console.log('\nan endless while loop must be caught, not freeze:');
elements['code-editor'].value = 'function laneCenterX(lane) { let i = 0; while (i >= 0) { i++; } return 0; }';
const t0 = Date.now();
elements['btn-test'].click();
...
check('all twelve steps are saved as done', JSON.parse(store['race-build-done']).length === 12);
...
check('typing in the editor does not steer the car', true);

console.log('\ncoming back later:');
check('the code was saved for every step',
      steps.every(s => typeof store['race-build-code-' + s.id] === 'string'));

console.log(failures === 0 ? '\nALL RACING WORKSHOP TESTS PASSED' : `\n${failures} WORKSHOP TEST(S) FAILED`);
